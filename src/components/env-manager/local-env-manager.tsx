...
export function LocalEnvManager() {
  ...
  const handleSave = async (data: APIKeyForm) => {
    ...
    const submitData = {
      ...data,
      ...Object.fromEntries(
        newApiKeys.map((entry) => [entry.key.trim(), entry.value.trim()]),
      ),
    };
  ...
  const handleAddNewKey = () => {
    setNewApiKeys([...newApiKeys, { key: '', value: '', id: uuidv4() }]);
  };
  ...
  const hasEmptyKeyValues = newApiKeys.some(
    (entry) => entry.key.trim() === '' || entry.value.trim() === '',
  );
  const hasDuplicateKeys = (): boolean => {
    const allKeys = [
      ...Object.keys(apiKeys || {}),
      ...newApiKeys.map((entry) => entry.key.trim()),
    ];
        ...
        <CardContent>
          <form onSubmit={handleSubmit(handleSave)} className="space-y-4">
            {keysArray &&
              keysArray?.map((key) => (
                <div key={key.id} className="space-y-2">
                  ...
                  {errors[key.id] && (
                    <p className="text-red-500">{errors[key.id]?.message}</p>
                  )}
                </div>
              ))}
            ...
            <div className="flex justify-between">
              <Button type="button" variant="outline" onClick={handleAddNewKey}>

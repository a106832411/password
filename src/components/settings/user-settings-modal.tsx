...
export function UserSettingsModal({
    ...
    const tabs: Tab[] = [
        ...
        { id: 'plan', label: '套餐', icon: Zap },
        { id: 'billing', label: '账单', icon: CreditCard },
        { id: 'usage', label: '使用量', icon: TrendingDown },
        ...(!isProduction ? [{ id: 'referrals' as TabId, label: '推荐奖励', icon: Users }] : []),
        { id: 'knowledge-base', label: '知识库', icon: FileText },
        { id: 'integrations', label: '集成', icon: Plug },
        { id: 'api-keys', label: 'API 密钥', icon: Key },
        ...(isLocal ? [{ id: 'env-manager' as TabId, label: '本地环境', icon: KeyRound }] : []),
    ];

...
export function getToolTitle(toolName: string): string {
  ...
  const toolTitles: Record<string, string> = {
    ...
    'crawl-webpage': '爬取网页',
    'scrape-webpage': '抓取网页',
    'browser-navigate-to': '浏览器导航',
    'browser-act': '浏览器操作',
    'browser-extract-content': '浏览器提取内容',
    ...
    'create-credential-profile': '创建凭证配置',
    ...
    'create-credential-profile-for-agent': '为智能体创建凭证配置',
    ...
    'create-event-trigger': '创建事件触发器',
    'create-agent-scheduled-trigger': '创建定时触发器',
    'list-agent-scheduled-triggers': '列出定时触发器',
    'delete-agent-scheduled-trigger': '删除定时触发器',
    'toggle-agent-scheduled-trigger': '切换定时触发器',

    'make-call': '发起呼叫',
    make_call: '发起呼叫',
    'end-call': '结束呼叫',
    end_call: '结束呼叫',
    'get-call-details': '呼叫详情',
    get_call_details: '呼叫详情',
    'list-calls': '呼叫记录',
    list_calls: '呼叫记录',
...
export function extractCommand(
  ...
  try {
    ...
    if (parsed.tool_calls && Array.isArray(parsed.tool_calls)) {
      ...
      if (execCommand && execCommand.function?.arguments) {
        try {
          const args =
            typeof execCommand.function.arguments === 'string'
  ...
  if (!contentStr.startsWith('{') && !contentStr.startsWith('[')) {
    ...
    if (
      !contentStr.includes('ToolResult') &&
      !contentStr.includes('No command')
    ) {
...
export function extractSessionName(
  ...
  try {
    ...
    if (parsed.tool_calls && Array.isArray(parsed.tool_calls)) {
      ...
      if (checkCommand && checkCommand.function?.arguments) {
        try {
          const args =
            typeof checkCommand.function.arguments === 'string'
  ...
  const sessionNameMatch = contentStr.match(
    /session_name["']?\s*[:=]\s*["']?([^"'\s]+)/i,
  );
  if (sessionNameMatch) {
...
export function extractStrReplaceContent(
  content: string | object | undefined | null,
): {
  oldStr: string | null;
  newStr: string | null;
...
export function extractFileContent(
  ...
  try {
    ...
    if (
      (toolType === 'create-file' || toolType === 'full-file-rewrite') &&
      parsedContent.file_contents
    ) {
    ...
    if (parsedContent.arguments) {
      const args =
        typeof parsedContent.arguments === 'string'
    ...
    if (parsedContent.content) {
      ...
      if (typeof parsedContent.content === 'object') {
        ...
        if (
          (toolType === 'create-file' || toolType === 'full-file-rewrite') &&
          parsedContent.content.file_contents
        ) {
...
export function extractUrlsAndTitles(
  ...
  const jsonObjectPattern =
    /\{\s*"title"\s*:\s*"([^"]+)"\s*,\s*"url"\s*:\s*"(https?:\/\/[^"]+)"\s*(?:,\s*"content"\s*:\s*"([^"]*)")?\s*\}/g;
  let objectMatch;
...
export function extractCrawlUrl(
  content: string | object | undefined | null,
): string | null {
  const contentStr = normalizeContentToString(content);
  if (!contentStr) return null;
  ...
  try {
    ...
    if (parsedContent.content) {
      if (typeof parsedContent.content === 'object') {
        ...
        return {
          title: content.Title || content.title || '网页内容',
      ...
      if (typeof parsedContent.content === 'string') {
        try {
          ...
          return {
            title: contentJson.Title || contentJson.title || '网页内容',
            text:
              contentJson.Text || contentJson.text || contentJson.content || '',
          };
        } catch (e) {
    ...
    if (typeof parsedContent === 'object' && parsedContent !== null) {
      ...
      if (
        ...
        'title' in parsedContent ||
        'Text' in parsedContent ||
        'text' in parsedContent
      ) {
        return {
          title: parsedContent.Title || parsedContent.title || '网页内容',
          text:
            parsedContent.Text ||
            parsedContent.text ||
...
export function extractSearchResults(
  ...
  try {
    ...
    const resultObjectPattern =
      /\{\s*"url":\s*"([^"]+)"\s*,\s*"title":\s*"([^"]+)"\s*,\s*"content":\s*"([^"]*)"[^}]*\}/g;
    const results = [];
    let match;
    ...
    try {
      ...
      // Handle case where content is a parsed object with content field that's also an object
        ...
        if (
          ...
          typeof content.content.content === 'string'
        ) {
      ...
      // Handle direct object that might be the content itself (new format)
        ...
        if (
          stringified.includes('file_path') ||
          stringified.includes('command')
        ) {
          return stringified;
        }
      ...
      console.error(
        'Error in normalizeContentToString:',
        e,
        'Content:',
        content,
      );
...
export function extractStreamingFileContent(
  ...
  if (typeof content === 'object' && content !== null) {
    try {
      if ('content' in content && typeof content.content === 'string') {
        ...
        const openTagMatch = content.content.match(
          new RegExp(`<${tagName}[^>]*>`, 'i'),
        );
        if (openTagMatch) {
          // Find where the tag ends
          const tagEndIndex =
            content.content.indexOf(openTagMatch[0]) + openTagMatch[0].length;
  ...
  if (openTagMatch) {
    // Find where the tag ends
    const tagEndIndex =
      contentStr.indexOf(openTagMatch[0]) + openTagMatch[0].length;
    // Extract everything after the opening tag
  ...
  switch (ext) {
    ...
    case 'webp':
      return {
        ...
        bgColor:
          'bg-gradient-to-br from-pink-500/20 to-pink-600/10 border border-pink-500/20',
      };
    ...
    case 'mdx':
      return {
        ...
        bgColor:
          'bg-gradient-to-br from-slate-500/20 to-slate-600/10 border border-slate-500/20',
      };
    case 'txt':
      return {
        icon: FileText,
    ...
    case 'mov':
      return {
        ...
        bgColor:
          'bg-gradient-to-br from-indigo-500/20 to-indigo-600/10 border border-indigo-500/20',
      };
    case 'mp3':
    case 'wav':
    case 'ogg':

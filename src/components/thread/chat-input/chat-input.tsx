...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    // Sync external value into the isolated textarea (e.g. when selecting a prompt example).
    // Note: ChatInput intentionally avoids notifying parent on every keystroke for perf;
    // this effect only reacts when the parent-provided value actually changes.
    useEffect(() => {
      if (!isControlled) return;
      if (controlledValue === undefined) return;

      // Avoid clobbering the user's current typing unless the external value actually changed.
      if (controlledValue === valueRef.current) return;

...
const PlanSelectionModal = lazy(() =>
  import('@/components/billing/pricing').then((mod) => ({
    default: mod.PlanSelectionModal,
  })),
);
const UpgradeCelebration = lazy(() =>
  import('@/components/billing/upgrade-celebration').then((mod) => ({
    default: mod.UpgradeCelebration,
  })),
);
...
const AgentRunLimitDialog = lazy(() =>
  import('@/components/thread/agent-run-limit-dialog').then((mod) => ({
    default: mod.AgentRunLimitDialog,
  })),
);
const CustomAgentsSection = lazy(() =>
  import('./custom-agents-section').then((mod) => ({
    default: mod.CustomAgentsSection,
  })),
);
const AgentConfigurationDialog = lazy(() =>
  import('@/components/agents/agent-configuration-dialog').then((mod) => ({
    default: mod.AgentConfigurationDialog,
  })),
);
const CreditsDisplay = lazy(() =>
  import('@/components/billing/credits-display').then((mod) => ({
    default: mod.CreditsDisplay,
  })),
);
...
export function DashboardContent() {
  ...
  const selectedAgent = selectedAgentId
    ...
    : null;
  const sunaAgent = agents.find(
    (agent) => agent.metadata?.is_suna_default === true,
  );
  ...
  const isDismissed =
    ...
    sessionStorage.getItem('threadLimitAlertDismissed') === 'true';
  const threadLimitExceeded =
    !isAccountStateLoading && !canCreateThread && !isDismissed;
  ...
  const hasDailyRefresh =
    dailyCreditsInfo?.enabled && dailyCreditsInfo?.seconds_until_refresh;

  const alertType =
    hasLowCredits && hasDailyRefresh
      ? 'daily_refresh'
      : threadLimitExceeded
        ? 'thread_limit'
        : null;
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (error instanceof ProjectLimitError) {
        ...
        const balanceMatch = originalMessage.match(
          /balance is (-?\d+)\s*credits/i,
        );

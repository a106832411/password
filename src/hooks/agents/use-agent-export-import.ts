...
export const useImportAgent = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      toast.success(
        `Agent "${variables.import_data.name}" ${action} successfully`,
      );
    },
    onError: (error: any) => {
...
export const validateImportData = (
  ...
  if (!data.export_version) {
    errors.push(
      'Export version is missing - this may not be a valid agent export file',
    );
  }
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!validation.isValid) {
          reject(
            new Error(`Invalid import file: ${validation.errors.join(', ')}`),
          );
          return;
        }

...
export const useAgents = (
  ...
  customOptions?: Omit<
    UseQueryOptions<
      ...
      Error,
      Awaited<ReturnType<typeof getAgents>>,
      ReturnType<typeof agentKeys.list>
    >,
  ...
  const normalizedParams = useMemo(() => {
    ...
    if (params.sort_order !== undefined)
      normalized.sort_order = params.sort_order;
    ...
    if (params.has_default !== undefined)
      normalized.has_default = params.has_default;
    if (params.has_mcp_tools !== undefined)
      normalized.has_mcp_tools = params.has_mcp_tools;
    if (params.has_agentpress_tools !== undefined)
      normalized.has_agentpress_tools = params.has_agentpress_tools;
    if (params.tools) normalized.tools = params.tools;
    if (params.content_type !== undefined)
      normalized.content_type = params.content_type;
  ...
  const placeholderData = useMemo(() => {
    ...
    const allAgentQueries = queryClient.getQueriesData<
      Awaited<ReturnType<typeof getAgents>>
    >({
    ...
    const isPaginated =
      normalizedParams.page !== undefined && normalizedParams.page > 1;
...
export const useUpdateAgent = () => {
  ...
  return useMutation<Agent, Error, { agentId: string } & AgentUpdateRequest>({
    mutationFn: ({
      agentId,
      ...data
    }: { agentId: string } & AgentUpdateRequest) => updateAgent(agentId, data),
 ...
 */
export const useAgentFromCache = (
  ...
  return useMemo(() => {
    ...
    const cachedAgent = queryClient.getQueryData<Agent>(
      agentKeys.detail(agentId),
    );

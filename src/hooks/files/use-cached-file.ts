...
export function useCachedFile<T = string>(
  ...
  const cacheKey =
    sandboxId && filePath ? getCacheKey(sandboxId, filePath) : null;
    ...
    try {
      ...
      const attemptFetch = async (
        ...
        if (!response.ok) {
          ...
          const isWorkspaceNotRunning = responseText.includes(
          ...
          console.error(
            `[FILE CACHE] Failed response for ${normalizedPath}: Status ${response.status}`,
          );
      ...
      const isOfficeFile = filePath
        ?.toLowerCase()
        .match(/\.(xlsx|xls|docx|doc|pptx|ppt)$/);
        ...
        case 'blob':
          ...
          if (isImageFile || isPdfFile) {
            ...
            if (isPdfFile && !blob.type.includes('pdf') && blob.size > 0) {
              console.warn(
                `[FILE CACHE] PDF blob has generic MIME type: ${blob.type} - will correct it automatically`,
              );
    ...
    try {
      ...
      if (cachedItem) {
        ...
        if (
          ...
          (options.expiration || CACHE_EXPIRATION)
        ) {
  ...
  return {
    ...
    getCachedFile: (key?: string, force = false) => {
      return key
        ? getCachedFile(key, force)
        : cacheKey
          ? getCachedFile(cacheKey, force)
...
export const FileCache = {
  ...
  set: (key: string, content: any) => {
    ...
    fileCache.set(key, {
      ...
      timestamp: Date.now(),
      type:
        typeof content === 'string' && content.startsWith('blob:')
          ? 'url'
          : 'content',
    });
  },
  ...
  getContentTypeFromPath: (path: string): 'text' | 'blob' | 'json' => {
    ...
    if (
      /^(xlsx|xls|docx|pptx|ppt|pdf|png|jpg|jpeg|gif|bmp|webp|svg|ico|zip|exe|dll|bin|dat|obj|o|so|dylib|mp3|mp4|avi|mov|wmv|flv|wav|ogg)$/.test(
        ext,
      )
    ) {
      return 'blob';
  ...
  getMimeTypeFromPath: (path: string): string => {
    ...
    switch (ext) {
      ...
      case 'doc':
        return 'application/json'; // TipTap document format
      case 'pptx':
        ...
        try {
          ...
          if (isBinaryFile) {
            ...
            if (FileCache.isImageFile(path)) {
              ...
              extension === 'pdf' ||
              ['xlsx', 'xls', 'docx', 'doc', 'pptx', 'ppt'].includes(
                extension || '',
              )
            ) {
  ...
  const attemptFetch = async (
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch file content: ${response.status} ${errorText}`,
        );
      }
      ...
      // Check if this is a workspace initialization error and we haven't retried yet
      const isWorkspaceNotRunning = error.message?.includes(
        'Workspace is not running',
      );
      if (isWorkspaceNotRunning && !isRetry) {
        await new Promise((resolve) => setTimeout(resolve, 2000));

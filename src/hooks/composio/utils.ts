...
export const composioApi = {
  ...
  async getToolkitIcon(
    ...
    // Icon fetching is no longer required; return a successful result without requesting backend.
    return { success: true };
  },
  ...
  async checkProfileNameAvailability(
    ...
    if (!result.success) {
      throw new Error(
        result.error?.message || 'Failed to check profile name availability',
      );
    }

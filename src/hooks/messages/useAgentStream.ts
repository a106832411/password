...
import { streamAgent, getAgentStatus, stopAgent } from '@/lib/api/agents';
...
export function useAgentStream(
  ...
  const handleStreamMessage = useCallback(
      ...
      switch (message.type) {
        case 'assistant':
          if (parsedMetadata.stream_status === 'tool_call_chunk') {
            ...
            callbacksRef.current.onAssistantChunk?.({
              content: parsedContent.content,
            });
    ...
    [finalizeStream, updateStatus, addContentThrottled, flushPendingContent],
  );

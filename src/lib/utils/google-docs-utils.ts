...
export async function downloadDocument(
  ...
  try {
    const response = await fetch(
      `${sandboxUrl}/document/convert-to-${format}`,
      {
        method: 'POST',
        headers: {
...
export const handleGoogleDocsAuth = async (
  docPath: string,
  sandboxUrl: string,
) => {
  try {
    sessionStorage.setItem(
      'google_docs_upload_intent',
      JSON.stringify({
        doc_path: docPath,
...
export const handleGoogleDocsUpload = async (
  sandboxUrl: string,
  docPath: string,
) => {
  ...
  try {
    ...
    const response = await backendApi.post(
      '/document-tools/convert-and-upload-to-docs',
      {
        doc_path: docPath,
        sandbox_url: sandboxUrl,
      },
    ...
    if (!response.success) {
      throw new Error(
        response.error?.message || 'Failed to upload to Google Docs',
      );
    }

...
import { getTokenFromStorage } from '@/lib/auth/jwt';
...
export const unifiedAgentStart = async (options: {
  ...
  try {
    ...
    const response = await backendApi.upload<{
      ...
      agent_run_id: string;
      status: string;
    }>('/agent/start', formData, { showErrors: false, cache: 'no-store' });
...
export const stopAgent = async (agentRunId: string): Promise<void> => {
  ...
  if (response.error) {
    const stopError = new Error(
      `Error stopping agent: ${response.error.message}`,
    );
...
export const generateAgentIcon = async (
  request: AgentIconGenerationRequest,
): Promise<AgentIconGenerationResponse> => {
  try {
    ...
    handleApiError(error, {
      operation: 'generate agent icon',
      resource: 'agent icon generation',
    });
    throw error;
  }
};

export const setupAgentFromChat = async (
  request: AgentSetupFromChatRequest,
): Promise<AgentSetupFromChatResponse> => {
  try {
    ...
    handleApiError(error, {
      operation: 'setup agent from chat',
...
export const getActiveAgentRuns = async (): Promise<ActiveAgentRun[]> => {
  try {
    ...
    if (response.error) {
      console.warn(
        `Failed to fetch active agent runs: ${response.error.status} ${response.error.message}`,
      );
      return [];
    }
...
export const streamAgent = (
  ...
  try {
    const setupStream = async () => {
      ...
      if (!token) {
        callbacks.onError('No authentication token available');
        callbacks.onClose();
        return;
      }

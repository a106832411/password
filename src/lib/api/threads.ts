...
import { getTokenFromStorage } from '../auth/jwt';
...
export const getProject = async (projectId: string): Promise<Project> => {
  ...
  if (response.error) {
    ...
    throw new Error(
      response.error.message || `Project not found: ${projectId}`,
    );
  }
...
export const deleteProject = async (projectId: string): Promise<void> => {
  ...
  if (!threadsResponse?.threads) {
    handleApiError(new Error('Failed to fetch threads'), {
  ...
  const deleteResponse = await backendApi.delete(
  ...
  if (deleteResponse.error) {
    handleApiError(deleteResponse.error, {
      operation: 'delete project',
      resource: `project ${projectId}`,
    });
  ...
  try {
    ...
    if (projectId) {
      threads = threads.filter(
        (thread: Thread) => thread.project_id === projectId,
      );
    }
...
export const getThreadsPaginated = async (
  projectId?: string,
  page: number = 1,
  limit: number = 20,
  ...
  try {
    ...
    const response = await backendApi.get<{ threads: any[]; pagination: any }>(
...
export const addUserMessage = async (
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/threads/${threadId}/messages/add`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',

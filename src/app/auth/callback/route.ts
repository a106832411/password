...
export async function GET(request: NextRequest) {
  ...
  // Derive a stable base URL that works behind proxies/load balancers
  ...
  const inferredOrigin = host
    ...
    : '';
  // Priority: explicit env → inferred origin from headers → Next's origin → localhost fallback
  const baseUrl =
    ...
    try {
      ...
      if (error) {
        ...
        if (isExpired) {
          // Redirect to auth page with expired state to show resend form
          const email = searchParams.get('email') || '';
          const expiredUrl = new URL(`${baseUrl}/login`);
          expiredUrl.searchParams.set('expired', 'true');
          if (email) expiredUrl.searchParams.set('email', email);
          if (next) expiredUrl.searchParams.set('returnUrl', next);
      ...
      if (data.user) {
        const pendingReferralCode = request.cookies.get(
        ...
        if (pendingReferralCode) {
          try {
            ...
            console.log(
              '✅ Added referral code to OAuth user:',
              pendingReferralCode,
            );
        ...
        if (accountData) {
          ...
          if (
            ...
            console.log(
              '⚠️ No subscription detected - redirecting to setting-up (fallback)',
            );
      ...
      if (shouldClearReferralCookie) {
        response.cookies.set('pending-referral-code', '', {
          maxAge: 0,
          path: '/',
        });
      }
